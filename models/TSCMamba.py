...
class Model(torch.nn.Module):
    ...
    def classification(self,x_cwt,batch_x_rocket,batch_x_raw):    
        ...
        if self.configs.no_rocket==1:
            x_projected=self.projector(batch_x_raw)
        elif self.configs.half_rocket==0:
            x_projected=batch_x_rocket
        else:
            x_mlp=self.projector(batch_x_raw)
            x_projected=torch.cat([batch_x_rocket[:,:,:self.configs.projected_space//2],
                                   x_mlp[:,:,self.configs.projected_space//2:]
                                ],dim=2)
